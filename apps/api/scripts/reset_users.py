...
def reset_users() -> None:
    ...
    try:
        ...
        numbers_stmt = update(RaffleNumber).values(
            status="available", taken_by_user_id=None, taken_at=None
        )
        ...
        db.commit()
        print(
            "Reseteo de usuarios completado con éxito. No queda ningún usuario y todos los datos asociados fueron limpiados."
        )
...
if __name__ == "__main__":
    ...
    print(
        "¡ATENCIÓN! Este script BORRARÁ TODOS LOS USUARIOS de la base de datos y limpiará sus participaciones."
    )
    if app_env == "production":
        print("¡¡ALERTA MÁXIMA!! Detectado entorno de PRODUCCIÓN.")
        confirm = input(
            "Para continuar y ELIMINAR TODOS LOS USUARIOS REALES, escribe exactamente 'ELIMINAR TODOS LOS USUARIOS': "
        )
        ...
        confirm = input(
            "¿Estás seguro de que deseas eliminar todos los usuarios de desarrollo? (escribe 'si' para continuar): "
        )
        if confirm.lower() != "si":
